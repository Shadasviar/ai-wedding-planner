...
export function EditServiceModal({ service, isOpen, onClose, onSave }: EditServiceModalProps) {
  ...
  useEffect(() => {
    if (service) {
      ...
      setCost(service.cost || 0)
      setPaidAmount(service.paidAmount || 0)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || "Nie udało się zaktualizować usługi")
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
        <h2 className="text-xl font-semibold text-zinc-900">Edytuj usługę</h2>
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          <div>
            <label htmlFor="edit-cost" className="block text-sm font-semibold text-zinc-800">
              Całkowity koszt (zł)
            </label>
            <input
              id="edit-cost"
          ...
          <div>
            <label htmlFor="edit-paidAmount" className="block text-sm font-semibold text-zinc-800">
              Już opłacono (zł)
            </label>
            <input
              id="edit-paidAmount"
          ...
          <div>
            <label htmlFor="edit-notes" className="block text-sm font-semibold text-zinc-800">
              Notatki
            </label>
            <textarea
              id="edit-notes"
          ...
          <div>
            <label htmlFor="edit-deadline" className="block text-sm font-semibold text-zinc-800">
              Termin
            </label>
            <input
              id="edit-deadline"
