...
interface AddCateringMenuItemModalProps {
  isOpen: boolean
  onClose: () => void
  onRefresh: () => void
}

export function AddCateringMenuItemModal({ isOpen, onClose, onRefresh }: AddCateringMenuItemModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/catering-menu-items", {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || "Nie udało się dodać dania")
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
        <h2 className="text-xl font-semibold text-zinc-900">Dodaj danie</h2>
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          <div>
            <label htmlFor="type" className="block text-sm font-semibold text-zinc-800">
              Typ dania
            </label>
            <select
              id="type"
          ...
          {type === "inne" && (
            <div>
              <label htmlFor="customType" className="block text-sm font-semibold text-zinc-800">
                Własny typ dania
              </label>
              <input
                id="customType"
          ...
          <div className="mt-6 flex gap-3">
            ...
            >
              {isSubmitting ? "Dodawanie..." : "Dodaj danie"}
            </button>
          </div>
        </form>
      </div>
